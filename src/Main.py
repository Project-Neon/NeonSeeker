...
def retorno(t):#função para o retorno
    ...
    cor=Get_cor.cor_mais_proxima(Get_cor.Sensor_direita.rgb)
...
def sair_da_cor_atual(cor):#TROCAR PELO ALINHAMENTO
    while Get_cor.cor_mais_proxima(Get_cor.Sensor_direita.rgb)==cor:
        rodas.on(-20,-20)
    rodas.off()
...
def andar_frente():#Corrigir todos os tempos presentes aqui a fim de utilizar com o robo e pista finais
    ...
    while 1:
        c=Get_cor.cor_mais_proxima(Get_cor.Sensor_direita.rgb)
        if c!="White" and c!="Black" and start_time==0:
            ...
            Plaza.verificar_plaza()#trocar por função de indentificação do plaza
            rodas.off()
...
def diferente_de(*cor):
    if Get_cor.cor_mais_proxima(Get_cor.Sensor_direita.rgb) not in cor:
        return 1
    else: return 0
...
if __name__=="__main__":
    ...
    while (1):
        tempo = andar_frente()
        if (Get_cor.cor_mais_proxima(Get_cor.Sensor_direita.rgb)=='Black'):#se ver Preto retorna até o ponto de referencia de onde saiu
            retorno(tempo)
