...
#DECLARA ^g ^cO DE VARIAVEIS GLOBAIS
...
#FUN ^g ^tES DE LOCOMO ^g ^bO
def retorno():#fun    o para o retorno
    ...
    procurar_proximo()#vira conforme as orienta    es que s  o possiveis
    alinha(0.02,230,30)#anda um pouco a frente para nao o robo n  o reconhecer o mesmo ponto de referencia como um novo ponto
...
def andar_frente():#Corrigir todos os tempos presentes aqui a fim de utilizar com o robo e pista finais
    ...
    #Vai para frente at   ver Black, retorna o tempo percorrido
    while 1:
        if(c=="Black"):
            rodas.off()
            retorno()
            ...
            if(Confirmar_cor(c)):
               ...
               cor_atual=c
                ...
                return
        while c=='White':
            #Anda pelo branco em procura do boneco se a mochila nao esta carregada(mochila==0).Sen  o apenas anda para frente no branco
            procurar_passageiro()
...
def procurar_proximo():#fun    o de virar conforme o aprendido, ou a falta dele
...
#FIM DAS FUN ^g ^tES DE LOCOMO ^g ^bO

#FUN ^g ^uES DE COR
def media(leitura1, leitura2):  # FAZ A M ^hDIA DAS LEITURAS DOS AMBOS SENSORES
    media = []
    for x in range(3):
...
def cor_mais_proxima(leitura):
    ...
    for valor in cores.values():
        ...
        if(dist < min):  # verifica se    menor que o ultimo verificado
             min = dist
             for key, value in cores.items():  # pega o nome da cor que gerou a menor distancia
                 if value == valor:
                     cor = key
    return cordef diferente_de(*cor):
    global c
    if c not in cor:
        return 1
    else: return 0
...
def verificar_plaza():
    ...
    if(1):
        if c!='Black':
            ...
            if(mudanca >= 2):
                print("PLAZA")
                pickle.dump(quads,open('memoria.p','wb'))#Armazena os valores aprendidos para debugs futuros
...
def Volta():
    ...
    i=len(quads)-1#Indice para o robo ir somente at   o ultimo quadrado
    while(i>0 and mochila==False):#Se quiser que o robo v   at   o ultimo: Tire a condi    o da mochila
        if c!='White':
            print(memoria_cor[c])
        ...
        if(mochila==True ):
            ...
            break
        i-=1
            #if sensor detectar algo retorna start_time e execute a fun    o de pegar o boneco
    if(i==0):
        virar(90)
        virar(90)
        while(c!='White'):rodas.on(-SpeedPercent(velocidade),-SpeedPercent(velocidade))
    ...
    procurar_passageiro()
        verificar_plaza()
#FIM DAS FUN ^g ^uES DO PLAZA

#FUN ^g ^uES DA MOCHILA(EQUIPAMENTO DE CAPTURAR BONECO)
...
#FIM DAS FUN ^g ^uES DE MOCHILA

#FUN ^g ^uES DE INFORMA ^g ^cO
class quad:#objeto que guarda informa    es do ponto de referencia encontrado
...
if __name__=="__main__":
    ...
    while (1):
        andar_frente()
        # if (tuts=0):#se ver Preto retorna at   o ponto de referencia de onde saiu
        #     retorno()
        # # se ver um novo ponto de referencia atualiza a memoria de tal cor, coloca na lista informa    es relativas ao descoberto e ao ulti$
        # if (tuts=1):
        #     print ('Achei: ',c)
        #     tentativa=0#reseta a variavel tentativas o que indica que    um novo quadrado
